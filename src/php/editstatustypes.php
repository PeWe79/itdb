				...
				<?php 
					...
					while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
						...
						if ($dbid>$internaltypes) //change this to remove X from internal types
							echo "
				<tr>
					<td title='Delete'><a href='javascript:delconfirm(\"$itype\",\"$scriptname?action=$action&amp;deltype=$dbid\");'>".
								"<img src='images/delete.png' border=0></a></td>";
						else
							echo "
				<tr>
					<td title='internal type ($dbid), cannot delete'></td>";
						$x=attrofstatus($dbid,$dbh);
						$attr=$x[0];
						$statustxt=$x[1];
						echo "
					<td nowrap><input type=hidden name='id[]' value='".$r['id']."' readonly size=3>";
					...
					echo "
				<tr>
					<td><input type=hidden name='id[]' value='new' readonly size=3>New:</td>";
					echo "
					<td><input size=15 maxlen=20 type=text name='statusdesc[]' ></td>";
					echo "
				</tr>";

				?>
				<tr>
					<td colspan=2><button type="submit"><img src="images/save.png" alt="Save" > <?php te("Save");?></button></td>
				</tr>
			</table>
		</form>
