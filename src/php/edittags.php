...
?>
		...
		<div style='float:left'>
			<table border=0 class='brdr' >
				<tr>
					<th>&nbsp;</th>
					<th><?php te("TAG");?></th>
					<th><?php te("Associated Items");?></th>
					<th><?php te("Associated Software");?></th>
				</tr>
				...
				<?php 
					...
					for ($i=0;$i<count($tags);$i++) {
						...
						echo "
				<tr>
					<td><a href='javascript:delconfirm(\"$name\",\"$scriptname?action=edittags&amp;delid=$dbid\");'><img title='delete' src='images/delete.png' border=0></a></td>";
						else echo "<tr><td>--</td>";

						echo "<td><input size='30' type='text' name='names[]' ".
							"value=\"".$tags[$i]['name']."\">";
						echo "<input type=hidden name='ids[]' value='$dbid' ></td>";
		...
		<div style='text-align:left;float:left;margin-left:50px;min-width:350px;max-width:500px;min-height:300px;border:1px solid #fff;' 
			id='itemresults'><?php te("Click on Item count column on the left to display associated items");?>
		</div>

		<div style='text-align:left;float:left;margin-left:50px;min-width:350px;max-width:500px;min-height:300px;border:1px solid #fff;' 
			id='softwareresults'><?php te("Click on Software count column on the left to display associated software");?>
		</div>
