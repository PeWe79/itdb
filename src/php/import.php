...
<div style='width:100%;'> <!-- import1 -->
  ...
  <?php if ($nextstep == 0) { ?>
    ...
    <table>
      <form method=post name='importfrm' action='<?php echo $scriptname ?>?action=<?php echo $action ?>' enctype='multipart/form-data'>
        ...
        <tr>
          ...
          <td> <input size=1 type=text name='delim' value=';' maxlength=1> (field separator)</td>
        </tr>
        <tr>
          <td>Skip 1st row:</td>
          <td><select name=skip1st>
              <option value=1>Yes</option>
              <option value=0>No</option>
            </select></td>
        </tr>
        <tr>
          <td colspan=2><input type=submit value='Upload file and inspect fields'></td>
        </tr>
    ...
    ?>
      ...
      <br>
      <h2> Please check fields for consistency before submiting</h2>
      <div style='height:400px;overflow:auto'>
        ...
        function lineok($line, $delim)
        {
          ...
          if (
            !strlen($cols[$name2fno['ip']])  //ip
            && !strlen($cols[$name2fno['manufacturer']]) //manufact
            && !strlen($cols[$name2fno['model']])
          ) { //model
            echo "\n";
