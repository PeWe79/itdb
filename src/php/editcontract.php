		...
		<?php 
			...
			//delete agent
			if (isset($_GET['delid'])) {
				//if we came from a post (save) the update agent 
				$delid=$_GET['delid'];
			...
			if (isset($_POST['id'])) {
				//if we came from a post (save) then update agent 
				$id=$_POST['id'];
				...
				if ($missing) {
					echo "$missing<a href='javascript:history.go(-1);'>Go back</a></body></html>";
					exit;
				}
		...
		<form method=post action='$scriptname?action=$action&amp;id=$id' id='mainform' enctype='multipart/form-data'  name='contractfrm'>";
			//echo "<form method=post action='x.php' enctype='multipart/form-data'  name='contractfrm'>";
		...
		?>
					...
					<div id="tab1" class="tab_content">
						<table class='tbl1' style='width:100%' >
							...
							<tr>
								...
								<td>
									<input  class='input1' size=20 type=text name='totalcost' value="<?php echo $r["totalcost"]?>">
								</td>
							</tr>
							<tr>
								<?php  
								...
								<td>
									<input  class='dateinp mandatory' id=startdate validate="required:true " size=10 title='<?php echo $datetitle?>' type=text id='startdate' name='startdate' value='<?php echo $sdate?>'> <!-- id is for validation to work with date selectors -->
								</td>
							</tr>
							<tr>
								<?php  
								...
								<td><input  class='dateinp mandatory' validate='required:true' size=10 title='<?php echo $datetitle?>' type=text id='currentenddate' name='currentenddate' value='<?php echo $edate?>'></td>
							</tr>
						</table>
					</div>
							...
							<div class="scrltblcontainer3">
								<table class=tbl2 id="renewalstable">
									...
									<tr> 
										...
										<td>
											<input class='dateinp' style='width:7em'  type="text" name="ren_enddatebefore[]" size="8" value='<?php echo $enddatebefore?>'>
										</td>

										<td>
											<input class='dateinp'  style='width:7em' type="text" name="ren_enddateafter[]" size="8" value='<?php echo $enddateafter?>'>
										</td>

										<td>
											<input class='dateinp'  style='width:7em' type="text" name="ren_effectivedate[]" size="8" value='<?php echo $effectivedate?>'>
										</td>

										<td><input type=text  name="ren_notes[]"  style='width:10em;' value='<?php echo $notes?>'></td> 
										<td>
											<input class='dateinp'  style='width:7em' type="text" name="ren_dateentered[]" size="8" value='<?php echo $dateentered?>'>
										</td>
			...
			<div id='tab6' class='tab_content'>
				<table class="tbl2" width='100%'>
					<tr>
						<td colspan=2><h2>Upload a File</h2></td>
					</tr>
					<tr>
						<td class="tdc">
		...
		<table>
			<tr>
				...
				<?php 
					echo "<td><button type='button' onclick='javascript:delconfirm2(\"{$r['id']}\",\"$scriptname?action=$action&amp;delid={$r['id']}\");'>".
						"<img title='delete' src='images/delete.png' border=0> Delete Contract</button></td></tr>";
					echo "</table>";
				...
				<form name="evteditfrm" method="post" id="evteditfrm" action="php/contractevents.php">
					<table>
						<tr>
							<td>ID:</td>
							<td><input type=text name='eventid' value='new' readonly></td>
						</tr>
						<tr>
