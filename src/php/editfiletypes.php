				...
				<table class=brdr>
					...
					<?php 
						...
						if ($dbid>$internaltypes) 
							echo "
					<tr>
						<td title='Delete [ID: ".$dbid."]'><a href='javascript:delconfirm(\"[ID: {$dbid}] $dbtypedesc\",\"$scriptname?action=$action&amp;deltype=$dbid\");'>".
								"<img src='images/delete.png' border=0></a></td>";
						else
							echo "
					<tr>
						<td title='internal type ($dbid), cannot be deleted or changed.'></td>";
					...
					<tr>
						...
						echo "
						<td><input size=15 maxlen=20 type=text name='typedesc[]' ></td>";
						echo "
					</tr>";
					?>
