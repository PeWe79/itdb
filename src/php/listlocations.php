		...
		<h1>
			...
			<a title='<?php te("Add new Location");?>' href='<?php echo $scriptname?>?action=editlocation&amp;id=new'><img border=0 src='images/add.png' ></a>
		</h1>
		...
		<div class='scrtblcontainerlist'>
			<table class='display' id='locationslisttbl'>
				...
				<tbody>
					...
					<tr>";
							echo "
						<td><a class='editid' href='$scriptname?action=editlocation&amp;id=".$r['id']."'>{$r['id']}</a></td>";
							echo "
						<td>{$r['name']}</td>";
							echo "
						<td>{$r['floor']}</td>";
