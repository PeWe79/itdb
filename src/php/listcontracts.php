		...
		<h1>
			...
			<a title='<?php te("Add new Contract");?>' href='<?php echo $scriptname?>?action=editcontract&amp;id=new'><img border=0 src='images/add.png' ></a>
		</h1>
		<table class='display' width='100%' id='contlisttbl'>
			...
			<tbody>
				...
				<tr id='trid{$r['id']}'>";
						echo "
					<td><a class='editid' href='$scriptname?action=editcontract&amp;id=".$r['id']."'>{$r['id']}</a></td>\n";
						echo "
					<td>{$r['parentid']}</td>\n";
						echo "
					<td>{$r['name']}</td>\n";
					...
					<td>{$r['title']}</td>\n";
						echo "
					<td><span title='{$r['startdate']}'></span>".date($dateparam,$r['startdate'])."</td>\n";
						echo "
					<td><span title='{$r['currentenddate']}'></span>".date($dateparam,$r['currentenddate'])."</td>\n";
						echo "
				</tr>\n";
					}
				?>
