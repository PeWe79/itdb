...
//delete file
if (isset($_GET['delid'])) {
	//if we came from a post (save) the update file 
	$delid=$_GET['delid'];
	if (!is_numeric($delid)) {
...
if (isset($_POST['id'])) {
	//if we came from a post (save), update the file 
	$id=$_POST['id'];
	$title=$_POST['title'];
	$type=$_POST['type'];
	...
	if ($_POST['id']=="new")  {
		//if we came from a post (save) the add software 
...
echo "<form id='mainform' method=post  action='$scriptname?action=$action&amp;id=$id' enctype='multipart/form-data'  name='addfrm'>";

?>

			<?php 
				if ($id=="new")
				echo "<h1>".t("Add File")."</h1>";
				else
				echo "<h1>".t("Edit File")."</h1>";
						...
						<table class="tbl2" style='width:300px;'>
								...
								<td>
									<select class='mandatory' validate='required:true' name='type'>
										<?php 
											...
											echo "<option $s value='$dbid'>$ftypedesc</option>";
										}
											}
										?>
							...
							<tr>
								...
								<td><input class='input2 dateinp mandatory' validate='required:true' id='date' size=20 type=text name='date' 
									value="<?php  if (!empty($r['date'])) echo date($dateparam,$r['date'])?>">
								</td>
							</tr>
							<tr>
					...
					<td class="tdtop">
						<table class="tbl2" width='90%'>
							...
							<tr> 
								...
								<td><input validate='required:<?php echo $file_validate_required;?>' name="file" id="file" size="25" type="file"></td>
							</tr>
						</table>
						<?php echo $tip?>
													...
													<tbody>
														<?php 
															...
															while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
																...
																echo " <tr><td><input name='itlnk[]' value='".$r['id'].
												...
												<table width='100%' class='sortable brdr' id='softwarelisttbl'>
													...
													<tbody>
														<?php 
															...
															while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
																if (isset($id) && ($r['id']==$id)) continue; //dont link recursively 
																echo " <tr><td><input name='softlnk[]' value='".$r['id'].
																"' checked type='checkbox' /></td>".
												...
												<table width='100%' class='sortable brdr' id='contractslisttbl'>
													...
													<tbody>
														<?php 
															...
															while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
																if (isset($id) && ($r['id']==$id)) continue; //dont link recursively 
																echo " <tr><td><input name='contrlnk[]' value='".$r['id'].
																"' checked type='checkbox' /></td>".
