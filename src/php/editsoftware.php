	...
	<?php 
		...
		if (isset($_POST['id'])) {
			//if we came from a post (save) the update software 
			$id=$_POST['id'];
	...
	?>
							...
							<td class="tdtop">
								<table class="tbl2" style='width:300px;'>
											...
											<select validate='required:true' class='mandatory' name='manufacturerid'>
												...
												<?php 
													foreach ($agents as $a) {
														...
														echo "
												<option $s value='$dbid' title='$dbid'>$atype</option>";
													}
													echo "
											</select>";
									...
									<tr>
										...
										<td><input validate='required:true' class='mandatory dateinp' size=10 title='<?php echo $datetitle?>' type=text name='purchdate' id='purchdate' value='<?php echo $d?>'></td>
									</tr>
									<tr>
					...
					<table>
						...
						<tr>
							<td colspan=3 class='tdc' >
										...
										<tbody>
											<?php 
												while ($ir=$sth->fetch(PDO::FETCH_ASSOC)) {
													...
													echo "
											<tr>
												<td><input name='softlnk[]' value='".$ir['id']."' ";
								...
								<sup>1</sup>
								<?php te("Select systems where this software is currently installed. Only items with 'software support' in their item type are shown.");?>
							...
							<tbody>
								<?php 
									while ($ir=$sth->fetch(PDO::FETCH_ASSOC)) {
										...
										echo "
								<tr>
									<td><input name='contrlnk[]' value='".$ir['id']."' ";
										if ($ir['islinked']) echo " checked ";
										echo  " type='checkbox' /></td>
			...
			<table>
				<tr>
					...
					echo "
					<td>
						<button type='button' onclick='javascript:delconfirm2(\"{$r['id']}\",\"$scriptname?action=$action&amp;delid={$r['id']}\");'>".
