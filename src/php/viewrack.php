...
if (isset($items) && $rack['revnums']) { //reverse rack row numbering
  foreach ($items as $it) {
    ...
    for ($pos = $it['rackposition']; $pos < ($it['rackposition'] + $it['usize']); $pos++) {
      ...
      if ($pos == $it['rackposition']) $isitemtop = 1;
      else $isitemtop = 0;
      if ($it['rackposdepth'] & 4) {
        $rackrow[$pos]['F'] = $it['id'];
        $rackrow[$pos]['FT'] = $isitemtop;
      }
      if ($it['rackposdepth'] & 2) {
        $rackrow[$pos]['M'] = $it['id'];
        $rackrow[$pos]['MT'] = $isitemtop;
      }
      if ($it['rackposdepth'] & 1) {
        $rackrow[$pos]['B'] = $it['id'];
        $rackrow[$pos]['BT'] = $isitemtop;
      }
    } //for usize
...
<div style='float:left;padding-left:10px;'>
  <table class=rack>
      ...
      for ($rr = 1; $rr <= $rack['usize']; $rr++) {
        ...
        if ($cell == 2) { //we have already printed one talbe cell in this row
          if ($rackrow[$rr]['MT']) { //is top row of this item?
            ...
            if ($hid == $rackrow[$rr]['M']) $c = "highlight";
            else $c = "occupied";
    ...
    <tr>
      <td style='border:0'>
      <td style='padding:0;border:0;text-align:center'><img height=30 src='images/rackwheel.png'></td>
      <td style='border:0'>
      <td style='padding:0;border:0;text-align:center'><img height=30 src='images/rackwheel.png'></td>
    </tr>
  </table>
